...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@noodzk'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('noodzk'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await shahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | WN")
                break
        ...
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | WN")
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | WN")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
    ...
    while True:
        try:
           ...
           if sender.id == onerabbas_id:
               ...
               joinu = await abbas(JoinChannelRequest('noodzk'))
               channel_entity = await abbas.get_entity(pot)
               await abbas.send_message(pot, '**جاري بدأ عملية التجميع بواسطة ويردو**')
               await abbas.send_message(pot, '/start')
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('noodzk'))
